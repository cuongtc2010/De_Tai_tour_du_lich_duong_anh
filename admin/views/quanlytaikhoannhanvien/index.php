...
<!--                        <center>
                        ...
                        <thead>
                            <tr role="row">
                                ...
                                <th class="sorting" tabindex="0" aria-controls="myTable" rowspan="1" colspan="1" >Tên nhân viên</th>
                                <th class="sorting" tabindex="0" aria-controls="myTable" rowspan="1" colspan="1" >Tên đăng nhập</th>
                               
                                <th class="text-center sorting" tabindex="0" aria-controls="myTable" rowspan="1" colspan="1" >SDT</th>
                                <th class="text-center sorting" tabindex="0" aria-controls="myTable" rowspan="1" colspan="1" >Trạng thái</th>
                            ...
                            if (!is_null($list)) {
                                ...
                                foreach ($list as $value) {
                                    ...
                                    <tr>
                                        ...
                                        <td><?= $i++ ?></td>
                                        <td><a style="color: #2196F3;" href="index.php?ctl=<?= $root_ctl ?>&act=update&id=<?= $value["Id"] ?><?= !empty($value["NhanVienId"]) ? "&NVId={$value["NhanVienId"]}" : "" ?>" ><?= $value["TenNV"] ?></a></td>
                                        <td class="text-right"><?php echo $value["username"] ?></td>
                                        <td class="text-right"><?php echo $value["SDT"] ?></td>
                                       <?php 
                                        $xacnhan = "Không hoạt động"; 
                                        if($value["Xoa"]  == 0){
                                            $xacnhan = "Hoạt động";
                                        }
                                        ?>
                                        <td style="color: <?= $value["Xoa"]  == 0 ? "#23b22e" : "#fe0000" ?>"><?= $xacnhan ?></td>
                                    </tr>
                                    <?php
                                }
                            }
